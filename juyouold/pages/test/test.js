    ...
    wx.getSetting({//get授权
      ...
      if (res.authSetting['scope.userInfo']) {//if授权？
        ...
        wx.getUserInfo({
          ...
          that.globalData.token = that.require_Get_Token("https://juyou.17lhd.com/juyou/api/userAccount/login?code=", that.globalData.code)//发起请求
            .then(res => {//--得到token后 更新个人信息 防止code6
              ...
              if(res.data.simpleUserInfo.nickName==null){
                that.requestPost("https://juyou.17lhd.com/juyou/api/userInfo/update?", param).then(res => {
                  console.log("修改用户信息")
                  console.log(res)
