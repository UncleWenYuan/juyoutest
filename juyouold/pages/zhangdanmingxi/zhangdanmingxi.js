...
,
  ...
  var app = getApp();
  var that = this;
  app.requestGet("https://juyou.17lhd.com/juyou/api/cashWithdrawal/list?").then(res => {
    //请求成功，do something
    console.log(res)
    that.setData({range:res.data.list})
  }).catch(res => {
  })
  app.requestGet("https://juyou.17lhd.com/juyou/api/recharge/list?").then(res => {
    //请求成功，do something
    console.log(res)
    that.setData({range2:res.data.list})
  }).catch(res => {
  })
  app.requestGet("https://juyou.17lhd.com/juyou/api/activityResult/incomeList?pageNum=1").then(res => {
    //请求成功，do something
    console.log(res)
